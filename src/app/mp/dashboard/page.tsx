...
function HealthDial({ score }: { score: number }) {
  const color = score > 75 ? '#4FFFB0' : score > 50 ? '#FFD60A' : '#FF3B5C'
  const r = 60
  ...
  return (
    <div className="relative w-48 h-28 mx-auto">
      <svg viewBox="0 0 160 90" className="w-full h-full">
        ...
        />
        {/* Labels */}
        <text x="20" y="78" fill="#4B5568" fontSize="9" fontFamily="JetBrains Mono">0</text>
        <text x="132" y="78" fill="#4B5568" fontSize="9" fontFamily="JetBrains Mono">100</text>
      </svg>
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-center">
        ...
        <div className="text-xs text-[#6B7A99] font-mono">HEALTH SCORE</div>
      </div>
    </div>
  )
}
...
const peerData = [
  { name: 'My Constituency', rate: constituency.completionRate, color: '#FF6B00' },
  { name: 'State Average', rate: 71, color: '#6B7280' },
  { name: 'National Average', rate: 69, color: '#6B7280' },
  { name: 'Top Performer', rate: 94, color: '#4FFFB0' },
]
    ...
    <div className="min-h-screen bg-[#04070F]">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-[#6B7A99] text-sm">{constituency.state} · MPLADS Allocation Overview</p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-4 gap-5">
          ...
          {[
            ...
            >
              ...
              <div className="text-xs text-[#6B7A99] mt-1">{m.label}</div>
            </motion.div>
          ))}
        </div>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-2.5 p-2.5 rounded-xl bg-danger/5 border border-danger/10 text-sm">
                      <span>{item.icon}</span>
                      <span className="text-[#A8B3CF]">{item.text}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="h-48">
                <ResponsiveContainer>
                  <BarChart data={peerData} layout="vertical" barCategoryGap={10}>
                    <XAxis type="number" domain={[0, 100]} tick={{ fill: '#4B5568', fontSize: 10, fontFamily: 'JetBrains Mono' }} />
                    <YAxis dataKey="name" type="category" width={130} tick={{ fill: '#A8B3CF', fontSize: 11 }} />
                    <ReferenceLine x={69} stroke="rgba(255,255,255,0.15)" strokeDasharray="4 2" label={{ value: 'Nat. Avg', fill: '#4B5568', fontSize: 9 }} />
                    <Tooltip
                      contentStyle={{ background: '#080D1A', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8 }}
        ...
        <div className="glass rounded-2xl border border-white/5 overflow-hidden">
          <div className="px-5 py-4 border-b border-white/5 flex items-center gap-2">
            ...
            <span className="ml-auto text-xs text-[#4B5568] font-mono">Citizen reports vs Official records</span>
          </div>
          <div className="p-5">
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="flex gap-6 mt-2 text-xs">
                    <div><span className="text-[#4B5568]">Official: </span><span className="text-forensic">72% Complete</span></div>
                    <div><span className="text-[#4B5568]">Community: </span><span className="text-caution">3 reports of incomplete</span></div>
                  </div>
                  <div className="flex items-center gap-3 mt-2 text-xs text-[#6B7A99]">
        ...
        <div className="glass rounded-2xl border border-white/5 p-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-[#6B7A99] mt-0.5">Generate monthly, quarterly, or annual performance reports</p>
          </div>
          <div className="flex gap-3">
            {['Monthly', 'Quarterly', 'Annual'].map(r => (
              <button key={r} className="px-4 py-2 rounded-xl border border-white/10 text-[#A8B3CF] text-xs hover:bg-white/5 hover:text-white transition-all flex items-center gap-1.5">
                <FileText size={12} /> {r}
              </button>
            ))}
          </div>
